...
def _build_live_session(session: ExamSession, db: Session) -> dict:
    ...
    risk = db.query(RiskScore).filter(RiskScore.session_id == session.id).first()
    viol_count = db.query(Violation).filter(Violation.session_id == session.id).count()
    ...
    live_score = (
        scorer.current_score() if scorer else (risk.current_score if risk else 0.0)
    )
    live_level = (
        scorer.current_level()
        if scorer
        else (risk.risk_level.value if risk else "SAFE")
    )
...
def dashboard(
    ...
    active_sessions = (
        db.query(ExamSession).filter(ExamSession.status == SessionStatus.ACTIVE).all()
    )
                    ...
                    if sid:
                        ...
                        try:
                            ...
                            if s:
                                ...
                                logger.warning(f"Admin WS terminated session={sid}")
                        finally:
                            db2.close()
