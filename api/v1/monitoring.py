...
Bugs fixed:
  ...
  2. All 3 still-broken lines corrected, everything else unchanged
"""
...
def process_frame(
    ...
    try:
        ...
        if not fd_result.detections:
            ...
            events.append(
                ViolationEvent("FACE_ABSENT", time.time(), 10, 1.0, 0.0, "face")
            )
            _save_violation(db, sid, "FACE_ABSENT", 10, 1.0)
        elif len(fd_result.detections) > 1:
            violations.append("MULTI_FACE")
            events.append(
                ViolationEvent("MULTI_FACE", time.time(), 30, 1.0, 0.0, "face")
            )
            ...
            try:
                ...
                if pv:
                    ...
                    _save_violation(
                        db, sid, "LOOKING_AWAY", 15, pr.confidence, pv.duration_seconds
                    )
            except Exception as e:
            ...
            try:
                ...
                for oe in oevs:
                    ...
                    _save_violation(
                        db, sid, vt, oe.weight, oe.confidence, screenshot=oe.frame_path
                    )
            except Exception as e:
            ...
            try:
                ...
                if gaze.region == "OFF_SCREEN":
                    events.append(
                        ViolationEvent("LOOKING_AWAY", time.time(), 8, 0.7, 0.0, "pose")
                    )
            except Exception as e:
            ...
            if msg_type == "FRAME" and scorer:
                try:
                        ...
                        if obj_det and int(time.time()) % 3 == 0:
                            try:
                                ...
                                for oe in oevs:
                                    ...
                                    _save_violation(
                                        ws_db, session_id, vt, oe.weight, oe.confidence
                                    )
                                    await websocket.send_json(
                                        {
                                            "type": "VIOLATION_DETAIL",
                    ...
                    for issue in liveness_issues:
                        ...
                        vtype, weight = vtype_map.get(
                            issue.issue_type, ("LIVENESS_ISSUE", 8)
                        )
                        ano_det.add_event(
                            ViolationEvent(
                                "WHISPER",
                ...
                if vtype:
                    ...
                    ano_det.add_event(
                        ViolationEvent(vtype, time.time(), w, prob, 0.0, "audio")
                    )
