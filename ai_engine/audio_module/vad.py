...
class HybridVADMonitor:
    ...
    #  Standalone demo
    # ─────────────────────────────────────────
    def run_demo(self, duration_seconds: float = 30.0):
        """python vad.py — live demo with pipeline efficiency display."""
        print(f"\n── Hybrid VAD Demo (WebRTC + Silero) — {duration_seconds}s ──")
        print("Speak into your mic. Ctrl+C to stop early.\n")
        print("  Stage 1: WebRTC  │  Stage 2: Silero  │  Result\n")
        ...
        try:
            while (time.time() - start) < duration_seconds:
                prob     = self.current_speech_prob()
                webrtc   = self._latest_webrtc
                silero   = self.is_speech_active()
                whisper  = self.is_whispering()

                w_icon = "W+" if webrtc  else "W-"
                s_icon = "S+" if silero  else ("SW" if whisper else "S-")
                ...
                skip_pct = self.stats.silero_skip_rate * 100
                fp_pct   = self.stats.false_positive_rate * 100

                print(
                    f"\r  [{w_icon}]─[{s_icon}]  [{bar}] {prob:.2f}  "
                    f"{result}  "
                    f"skip={skip_pct:.0f}%  "
                    f"FP={fp_pct:.0f}%  "
                    f"viol={self.stats.total_violations}  ",
                    end="", flush=True,
                )

                events = self.poll_violations()
                for e in events:
                    print(f"\n  ⚠  {e.violation_type} | {e.description}")
        ...
        print(f"  Speech duration   : {s['total_speech_seconds']}s")
        print(f"\n  Pipeline efficiency:")
        print(f"    Total frames    : {ps['total_frames']}")
        print(f"    WebRTC passed   : {ps['webrtc_passed']}  ({ps['webrtc_passed']/max(ps['total_frames'],1)*100:.1f}%)")
        print(f"    Silero skipped  : {ps['frames_skipped']}  (saved {ps['silero_skip_rate']*100:.1f}% CPU)")
        print(f"    Silero confirmed: {ps['silero_confirmed']}")
        print(f"    WebRTC FP rate  : {ps['false_positive_rate']*100:.1f}%  (caught by Silero)")
        print(f"\n  Violations:")
        for i, e in enumerate(s["violation_events"], 1):
            print(
                f"    [{i}] {e['type']:18s} | w={e['weight']:2d} | "
                f"prob={e['speech_prob']:.2f} | {e['description']}"
            )
        print("─────────────────────────────────────────────────────────\n")
...
if __name__ == "__main__":
    ...
    monitor = HybridVADMonitor()
    monitor.run_demo(duration_seconds=30.0)