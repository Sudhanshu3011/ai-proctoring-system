...
class FaceRecognizer:
    ...
    def verify_from_base64(
        ...
        try:
            embedding_bytes = base64.b64decode(embedding_base64)
            live_embedding = np.frombuffer(embedding_bytes, dtype=np.float32).copy()
    ...
    def search(
        ...
        logger.info(
            f"Search | best={best_user_id} " f"sim={best_sim:.4f} | {result.label}"
        )
        return result
    ...
    def start_session(
        ...
        logger.info(f"Session locked | session={session_id} user={user_id}")
        return sv
    ...
    def reverify_session(
        ...
        sim = self.cosine_similarity(live_embedding, sv.initial_embedding)
    ...
    def _load_registry(self):
        ...
        try:
            ...
            logger.info(f"Registry loaded: {len(self._registry)} faces " f"from {path}")
        except Exception as e:
