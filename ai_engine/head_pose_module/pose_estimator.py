...
class PoseEstimator:
    ...
    # ─────────────────────────────────────────
    def run_webcam(self):
        """Run directly:  python pose_estimator.py  — press ESC to quit."""
        cap = cv2.VideoCapture(0)
        if not cap.isOpened():
            logger.error("Cannot open webcam.")
            return

        logger.info("Head Pose Estimator running. Press ESC to stop.")
        last_viol: Optional[ViolationEvent] = None
        ...
        while True:
            ...
            viol   = self.check_violation(result)
            if viol:
                last_viol = viol
            if last_viol and time.time() - last_viol.timestamp > 2.0:
                last_viol = None

            self._draw_hud(frame, result, last_viol)
            cv2.imshow("Head Pose Estimator", frame)
        ...
        print(f"  High Freq Risk   : {s['high_frequency_risk']}")
        for i, e in enumerate(s["violation_events"], 1):
            print(f"  [{i}] {e['direction']:6s} | "
                  f"Yaw {e['yaw']:+.1f}deg Pitch {e['pitch']:+.1f}deg | "
                  f"Held {e['duration_seconds']}s")
        print("─────────────────────────────────────────────\n")
