...
class ReportService:
    ...
    def _build_cover(self, data: dict) -> list:
        ...
        elems.append(Paragraph("AI EXAM PROCTORING REPORT", s["ReportTitle"]))
        elems.append(
            Paragraph(
        ...
        info_data = [
            [
                ...
                data.get("user_name", "—"),
                "Session ID",
                str(data.get("session_id", "—"))[:16] + "...",
            ],
            ["Email", data.get("user_email", "—"), "Exam", data.get("exam_title", "—")],
            [
                "Started",
                _fmt_dt(started) if started else "—",
                "Submitted",
                _fmt_dt(submitted) if submitted else "—",
            ],
            [
                "Duration",
                _elapsed(started, submitted) if started else "—",
                "Risk Level",
                level,
            ],
        ]
    ...
    def _build_score_section(self, data: dict) -> list:
        ...
        if buf:
            ...
            elems.append(Paragraph("No score timeline data available.", s["SmallGray"]))
        return elems
    ...
    def _build_anomaly_section(self, data: dict) -> list:
        ...
        if not flags:
            elems.append(
                Paragraph("No behavioral anomaly patterns detected.", s["SmallGray"])
            )
            return elems
    ...
    def _build_violation_timeline(self, data: dict) -> list:
        ...
        for chunk_start in range(0, len(rows), chunk_size):
            chunk = (
                [rows[0]] + rows[chunk_start + 1 : chunk_start + 1 + chunk_size]
                if chunk_start > 0
                else rows[: chunk_size + 1]
            )
            ...
            high_weight_rows = [
                i + 1 for i, row in enumerate(chunk[1:]) if int(row[4]) >= 30
            ]
            style_cmds = [
                ("BACKGROUND", (0, 0), (-1, 0), C_ACCENT),
                ("TEXTCOLOR", (0, 0), (-1, 0), C_WHITE),
    ...
    session_data = {
        ...
        "module_stats": {
            "face": {
                "violation_count": 8,
                "total_weight": 80.0,
                "most_frequent": "FACE_ABSENT",
                "anomaly_detected": False,
            },
            "pose": {
                "violation_count": 15,
                "total_weight": 225.0,
                "most_frequent": "LOOKING_AWAY",
                "anomaly_detected": True,
            },
            "object": {
                "violation_count": 3,
                "total_weight": 120.0,
                "most_frequent": "PHONE_DETECTED",
                "anomaly_detected": True,
            },
            "audio": {
                "violation_count": 6,
                "total_weight": 100.0,
                "most_frequent": "SPEECH_BURST",
                "anomaly_detected": False,
            },
            "browser": {
                "violation_count": 3,
                "total_weight": 60.0,
                "most_frequent": "TAB_SWITCH",
                "anomaly_detected": False,
            },
        },
        "anomaly_flags": [
